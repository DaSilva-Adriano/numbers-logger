...
class AutoStopConfigTests(unittest.TestCase):
    ...
    def test_invalid_minutes_use_default(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "config.json"
            path.write_text(
                json.dumps({"auto_stop_minutes": "nope"}) + "\n",
                encoding="utf-8",
            )
            config = load_config(path)
            self.assertEqual(config.auto_stop_minutes, DEFAULT_AUTO_STOP_MINUTES)
...
class AutoStopElapsedTests(unittest.TestCase):
    def test_disabled_never_elapses(self) -> None:
        self.assertFalse(auto_stop_elapsed(0.0, enabled=False, minutes=1, now=120.0))

    def test_missing_start_never_elapses(self) -> None:
        self.assertFalse(auto_stop_elapsed(None, enabled=True, minutes=1, now=120.0))

    def test_before_duration(self) -> None:
        self.assertFalse(auto_stop_elapsed(0.0, enabled=True, minutes=1, now=59.9))

    def test_at_duration(self) -> None:
        self.assertTrue(auto_stop_elapsed(0.0, enabled=True, minutes=1, now=60.0))

    def test_after_duration(self) -> None:
        self.assertTrue(auto_stop_elapsed(10.0, enabled=True, minutes=2, now=130.0))

    def test_new_session_uses_new_start(self) -> None:
        self.assertFalse(auto_stop_elapsed(200.0, enabled=True, minutes=1, now=230.0))
        self.assertTrue(auto_stop_elapsed(200.0, enabled=True, minutes=1, now=260.0))
